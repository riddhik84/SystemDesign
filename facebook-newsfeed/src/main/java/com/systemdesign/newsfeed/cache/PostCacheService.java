 ...
 *
 * Key namespace:
 *   post:{postId}   → PostResponse JSON (TTL: 1 hour)
 *
 * Cache-aside pattern: read-through cache for post detail lookups.
 * When a post is created, it is immediately cached.
 * Posts are evicted on explicit evict() calls (e.g., on like/comment counter bumps).
 *
...
public class PostCacheService {
    ...
    /**
     * Evict a post from cache (e.g., on like/comment counter bumps).
     */
